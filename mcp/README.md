...
TodoList 的 Model Context Protocol (MCP) 服务实现，允许 AI 助手通过标准接口管理任务和工作区。
...
| 工具 | 描述 | 关键参数 |
|------|------|---------|
| `get_todos` | 获取任务列表 | `status?`, `tagId?`, `workspace?` |
| `create_todo` | 创建新任务 | `text`, `workspace?` |
| `update_todo` | 更新任务信息 | `id`, `text?`, `completed?`, `workspace?` |
| `delete_todo` | 删除任务 | `id` |
| `toggle_todo` | 切换任务完成状态 | `id` |
| `search_todos` | 模糊搜索任务标题 | `keyword`, `workspace?`, `status?` |
| `get_todos_by_tag` | 通过标签ID查询任务 | `tagId`, `workspace?`, `status?` |

### 工作区管理

| 工具 | 描述 | 关键参数 |
|------|------|---------|
| `get_workspaces` | 获取所有工作区 | 无 |
| `create_workspace` | 创建工作区 | `name`, `color?`, `path?` |
...
- 配置前请确保 TodoList 服务已启动（`npm run dev`）

**配置检查：**
```bash
# 测试 MCP 服务是否可访问
curl http://localhost:4001/mcp
# 应返回: {"name": "todolist-mcp-server", ...}
```

配置完成后，Kimi CLI 会自动连接到远程 MCP 服务，你可以在对话中直接使用：
...
- **服务依赖**：需要确保 TodoList 服务（Web + MCP）正在运行，MCP 默认使用 4001 端口
- **无状态设计**：每个操作都需要显式指定 `workspace` 参数，不提供会话状态
